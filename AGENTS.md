# NODAYSIDLE-Project-Pages — AGENTS.md
...
2. **Unit / fast checks**
   - `UNKNOWN` — no dedicated unit/test command was verified in `package.json`.
...
5. **Package / install**
   - `UNKNOWN` — static site artifact/deploy flow is provider-driven, not a local app package.
   - Deployment/provider changes require explicit NDI approval.

6. **Release gate**
   - Pushing branches is allowed only when requested.
   - Merging to `main`, publishing releases, changing deployment/provider settings, credential changes, and destructive cleanup require explicit NDI approval.
